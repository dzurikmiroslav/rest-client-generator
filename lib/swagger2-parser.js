...
Swagger2Parser.prototype.parse = function () {
    return util.fetchFile(this.options.input)
        .then(function (swaggerContent) {
            ...
            return {
                rootUrl: this.options.rootUrl || path.join(swaggerDef.host, swaggerDef.basePath),
    ...
    _.forEach(swaggerDef.paths, function (pathDef, pathKey) {
        _.forEach(pathDef, function (methodDef, methodKey) {
            ...
            methodDef.tags.every(function (tag) {
                var serviceName = util.format(tag, this.options.capitalize) + this.options.serviceSuffix;
...
Swagger2Parser.prototype.parseMethod = function (methodDef, httpMethod, path) {
    ...
    if (parmsDef.length) {
        method.queryParams = _.map(parmsDef, function (paramDef) {
            return {
                name: paramDef.name,
                type: getJsType(paramDef.type, paramDef.format) // array not supported
            };
        });
    }
        ...
        return {
            ...
            fields: _.map(typeDef.properties, function (propDef, propKey) {
                ...
                return {
                    name: propKey,
                    type: type,
                    optional: typeDef.required && typeDef.required.indexOf(propKey) >= 0,
                    array: array
                };
            }.bind(this))
        };
...
function getJsType(type, format) {
    switch (type) {
        case 'string':
